...
class IWAE(tf.keras.Model):
    ...
    def call(self, x, n_samples):
        ...
        # average over batch to get elbo
        ...
        # average over batch
        loss = -tf.reduce_mean(objective, axis=-1)
...
train_log_dir = "/tmp/iwae/task11/" + current_time + "/train"
val_log_dir = "/tmp/iwae/task11/" + current_time + "/val"
...
for epoch in range(epochs):
    ...
    for _step, _x_batch in enumerate(train_dataset):
        ...
        if step % 200 == 0:
            ...
            if val_elbo > best:
                print("saving model...")
                model.save_weights('/tmp/iwae/task11/best_weights' + '_nsamples_{}'.format(n_samples))
                best = val_elbo
...
# ---- save final weights
model.save_weights('/tmp/iwae/task11/final_weights' + '_nsamples_{}'.format(n_samples))
