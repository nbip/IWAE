...
val_log_dir = "/tmp/iwae/task01/" + current_time + "/val"
...
for epoch in range(epochs):
    ...
    for _step, x_batch in enumerate(train_dataset):
        ...
        if step % 200 == 0:
            ...
            if val_elbo > best:
                print("saving model...")
                model.save_weights('/tmp/iwae/task01/best_weights' + '_nsamples_{}'.format(n_samples))
                best = val_elbo
...
plt.ylim([-96, -90])
plt.savefig('task01_history' + '_nsamples_{}'.format(n_samples))
plt.close()

# ---- if you want to do early stopping based on the best validation set error:
model.load_weights('/tmp/iwae/task01/best_weights' + '_nsamples_{}'.format(n_samples))
