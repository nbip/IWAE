...
train_log_dir = "/tmp/iwae/task25/" + current_time + "/train"
train_summary_writer = tf.summary.create_file_writer(train_log_dir)
test_log_dir = "/tmp/iwae/task25/" + current_time + "/test"
...
for epoch in range(epochs):
    ...
    if epoch in plt_epochs:
        utils.generate_and_save_images(model, z2, epoch, "task25_{}_".format(n_samples))
...
# ---- save final weights
model.save_weights('/tmp/iwae/task25/final_weights' + '_nsamples_{}'.format(n_samples))

# ---- load the final weights?
# model.load_weights('/tmp/iwae/task25/final_weights' + '_nsamples_{}'.format(n_samples))
