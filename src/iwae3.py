...
class IWAE(tf.keras.Model):
    ...
    def call(self, x, n_samples, beta=1.0):
        ...
        # note that the above formulation works better than the one below
        # vae_elbo = tf.reduce_mean(log_w)
        # line 700: https://github.com/casperkaae/LVAE/blob/master/run_models.py
        # m = tf.reduce_max(log_w, axis=0)
        # vae_elbo = tf.reduce_mean(m) + tf.reduce_mean(tf.math.log(tf.reduce_mean(tf.exp(log_w - m), axis=0)))
