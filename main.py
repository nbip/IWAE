...
# ---- load data
Xtrain, Xval, Xtest, ytrain, yval, ytest = utils.create_binarized_mnist()
...
train_log_dir = "/tmp/iwae/" + current_time + "/train"
val_log_dir = "/tmp/iwae/" + current_time + "/val"
...
for epoch in range(epochs):
    ...
    for _step, x_batch in enumerate(train_dataset):
        ...
        if step % 200 == 0:
            ...
            if val_elbo > best:
                print("saving model...")
                model.save_weights('/tmp/iwae/best_weights' + '_nsamples_{}'.format(n_samples))
                best = val_elbo
...
model.load_weights('/tmp/iwae/best_weights' + '_nsamples_{}'.format(n_samples))
